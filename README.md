...
- 推奨エンドポイント（Tsubakuro カスタムサーバー）: `https://your-site.test/wp-json/tsubakuro/v1/mcp`
...
```json
{
  "mcpServers": {
    ...
    "wordpress-http-custom": {
      ...
      "env": {
        "WP_API_URL": "http://your-site.test/wp-json/tsubakuro/v1/mcp",
        "LOG_FILE": "/path/to/logs/mcp-adapter.log",
        "WP_API_USERNAME": "your-username",
        "WP_API_PASSWORD": "your-application-password"
      }
...
```sh
curl -X POST https://your-site.test/wp-json/tsubakuro/v1/mcp \
...
url = "https://your-site.test/wp-json/tsubakuro/v1/mcp"
...
claude mcp add --transport http tsubakuro \
  https://your-site.test/wp-json/tsubakuro/v1/mcp \
  --header "Authorization: Basic <Base64エンコードした認証情報>"
```

### GitHub Copilot / VS Code

ワークスペースの `.vscode/mcp.json` へ設定する場合は、上記 `mcpServers` 例の `wordpress-http-custom` を利用してください。

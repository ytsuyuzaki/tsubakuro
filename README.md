...
公開GitHub Releaseを更新元として、WordPress管理画面から通常のプラグイン更新を実行できます。更新チェックには Composer で管理する
[`plugin-update-checker`](https://github.com/YahnisElsts/plugin-update-checker) v5.7を使用します。ライブラリは `composer install` で `vendor/` に取得され、配布ZIPには `vendor/` を同梱します。

...
if ( ! WP_API_URL || ! WP_API_USERNAME || ! WP_API_PASSWORD ) {
	console.error(
		'Missing required environment variables: WP_API_URL, WP_API_USERNAME, WP_API_PASSWORD'
	);
	process.exit( 1 );
}
...
async function main() {
	...
	console.log(
		'initialize OK:',
		initialize.result?.serverInfo?.name || 'unknown-server'
	);
	...
	if ( ! toolNames.includes( 'tsubakuro-list-tasks' ) ) {
		throw new Error(
			`Expected tool not found. tools=${ JSON.stringify( toolNames ) }`
		);
	}

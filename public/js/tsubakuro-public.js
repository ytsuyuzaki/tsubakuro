...
var cfg           = tsubakuroPublic;
var REST          = cfg.restUrl;   // e.g. https://example.com/wp-json/tsubakuro/v1
...
$.ajax(opts).done(onSuccess).fail(onError || function () {
...
$('.tsubakuro-tab-content[data-tab="' + tab + '"]').addClass('active');
...
var $list = $('#tsubakuro-pub-task-list');
$list.html('<p class="tsubakuro-loading">読み込み中...</p>');

var params = {};
var statusFilter = $('#tsubakuro-pub-status-filter').val();
var pageFilter   = $('#tsubakuro-pub-page-filter').is(':checked');
...
var qs = $.param(params);
restRequest('GET', '/tasks' + (qs ? '?' + qs : ''), null, function (tasks) {
renderTaskList($list, tasks);
}, function () {
$list.html('<p class="tsubakuro-empty">タスクを読み込めませんでした。</p>');
});
}
...
var assigneeName = task.assignee ? task.assignee.display_name : '未アサイン';
var $item = $(
'<div class="tsubakuro-pub-task-item" data-task-id="' + escapeHtml(String(task.id)) + '">' +
'<div class="tsubakuro-pub-task-title">' + escapeHtml(task.title) + '</div>' +
'<div class="tsubakuro-pub-task-meta">' +
'<span class="tsubakuro-badge tsubakuro-badge--' + escapeHtml(task.status) + '">' + escapeHtml(task.status_label) + '</span>' +
...
restRequest('GET', '/tasks/' + taskId, null, function (task) {
renderDetail(task);
}, function () {
...
var $list = $('#tsubakuro-pub-comment-list');
$list.empty();

if (!comments.length) {
$list.html('<p style="color:#9ca3af;font-size:12px;padding:4px 0;">コメントはありません。</p>');
return;
}
...
'<div class="tsubakuro-pub-comment-meta">' + escapeHtml(c.user_name) + ' \u2013 ' + escapeHtml(c.created_at) + '</div>' +
...
var newStatus = $('#tsubakuro-pub-detail-status').val();

restRequest('POST', '/tasks/' + currentTaskId, { status: newStatus }, function () {
...
var comment = $.trim($('#tsubakuro-pub-comment-input').val());
if (!comment) { return; }

restRequest('POST', '/tasks/' + currentTaskId + '/comments', { comment: comment }, function () {
...
var editingId = $('#tsubakuro-pub-editing-id').val();
var titleV    = $.trim($('#tsubakuro-pub-title').val());
...
var relatedPages = [];
if ($('#tsubakuro-pub-link-page').is(':checked') && cfg.currentPage) {
...
restRequest('POST', path, body, function () {
...
// Filters
// =========================================================================
$(document).on('change', '#tsubakuro-pub-status-filter, #tsubakuro-pub-page-filter', function () {
loadTaskList();
});
...
var $notice = $('<div class="tsubakuro-pub-notice tsubakuro-pub-notice--' + type + '">').text(message);
